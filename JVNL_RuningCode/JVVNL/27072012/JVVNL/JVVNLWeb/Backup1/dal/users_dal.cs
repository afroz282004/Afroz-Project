     ...
     partial class users_dal
    {
        ...
        #region "User Data Methods"
            public string UsersCheckLogin(users_Prop userprop)
            {
                try
                {
                    DataSet ds= SqlHelper.ExecuteDataset(com.con, "usp_Users_checklogin", new object[] {userprop.UserName,userprop.Password,userprop.IPAddress   }) ;
            ...
            public DataTable UserSearch(users_Prop userprop)
            {
                try
                {
                return SqlHelper.ExecuteDataset(com.con, "usp_Users_Search", new object[] { userprop.UserName, userprop.GroupName ,userprop.CounterName  }).Tables[0];  
                }
                catch (Exception e)
                {
                    throw e;
