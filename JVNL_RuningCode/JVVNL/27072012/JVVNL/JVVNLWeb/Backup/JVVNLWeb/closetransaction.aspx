<%@ Page Language="C#" MasterPageFile="~/main.Master" AutoEventWireup="true" CodeBehind="closetransaction.aspx.cs" Inherits="JVVNLWeb.closetransaction" Title="Untitled Page" %>
							...
							<div class="block">
								<div class="form_place"  id="form1">
									<table class="fieldtable">
									    <tr>
									        <td>Date:</td>
									        <td><input   type="text" id="fdate" name="fdate" size="20" maxlength="10" readonly="readonly" value="<% Response.Write( DateTime.Today.ToString("dd/MM/yyyy")); %>"" /></td>
							...
							<div class="block">
								<div class="form_place"  >
									<table class="fieldtable">
									    ...
									    <tr>
									        <td>Cash Amount:</td>
									        <td><input type="hidden" id="username" value="<% Response.Write(Session["username"].ToString()); %>" /><input   type="text" value="0"  id="cashamount" name="cashamount" size="15"   /></td>
									        <td>Cheque Amount:</td>
									    ...
									    <tr>
									       ...
									       <input   type="text" value="0"  id="txtOneThosand" name="txtOneThosand" size="15"    onkeyup = "keyUP(event.keyCode)" onkeydown = "return isNumeric(event.keyCode);" onpaste = "return false;" />
									         <%--<input   type="text" value="0"  id="txtOneThosand" name="txtOneThosand" size="15" onKeyup="isInteger(this.value)"   onkeydown = "return isNumeric(event.keyCode);" onpaste = "return false;" />--%>
									    ...
									    <tr>
									       <td colspan="2">
									         <%--<input   type="text" value="0" id="txtFiveHundred" name="txtFiveHundred" size="15"  onKeyup="isInteger(this.value)" onkeydown = "return isNumeric(event.keyCode);" onpaste = "return false;"/>--%>
...
//          alert(parseInt(CashCollection));
//          alert(parseInt(totalamount));

        if (parseInt(CashCollection) == parseInt( totalamount)) {
            $.post("handler/DailyCollection.ashx?type=insert&txtOneThosand=" + OneThosand + "&txtFiveHundred=" + FiveHundred + "&txtHundred=" + Hundred + "&txtFifty=" + Fifty + "&txtTwenty=" + Twenty + "&txtTen=" + Ten + "&txtFive=" + Five + "&txtTwo=" + Two + "&txtOne=" + One + "&username=" + username, $("#form").serialize(), SaveData);
    ...
    function FillDialog(result) {
        ...
        {
            alert("Transaction not closed by user");
        }
    ...
    function FillCashSelect(result) {
        ...
        if (data.length > 1) {
            ...
            if (Mode == 'Cash') {
                document.getElementById("cashamount").value =Math.round(data[1]);
            }
            else {
                document.getElementById("chequeamount").value =Math.round(data[1]);
            }
    ...
    function PopulateCloseTransaction()
    {
       
        var fromdate= document.getElementById("fdate").value; 
        $.post("handler/payment.ashx?type=closetransaction&fromdate=" + fromdate , $("#form").serialize(), ShowTransaction);
    }
        function ShowTransaction(result)
    {
     ...
     function ShowTrans(username)
    {
        
        var fromdate= document.getElementById("fdate").value; 
        $.post("handler/DailyCollection.ashx?type=cashcollection&date="+ fromdate  +"&username=" + username , $("#form").serialize(),FillDialog);
